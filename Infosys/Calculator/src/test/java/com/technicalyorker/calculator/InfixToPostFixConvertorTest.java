...
public class InfixToPostFixConvertorTest {
	...
	public void simple() {
		...
		TestCase.assertEquals("3 4 + ", convertor.perform("3 + 4"));
	}

	@Test
	public void testBitComplexCase() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		TestCase.assertEquals("3 4 5 + + ", convertor.perform("3 + ( 4 + 5 )"));
	}
	...
	public void testSine() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		TestCase.assertEquals("30 Sine ", convertor.perform("Sine 30"));
	}

	@Test
	public void testSineWithProduct() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		TestCase.assertEquals("3 30 Sine * ", convertor.perform("3 * Sine 30"));
	}

	@Test
	public void exponents() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		TestCase.assertEquals("2 4 ^ ", convertor.perform("2 ^ 4"));
	}

	@Test
	public void testTan() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		TestCase.assertEquals("3 45 Tan * ", convertor.perform("3 * Tan 45"));
	}

	@Test(expected = InvalidInputOperandCalculatorException.class)
	public void testNegative() {
		InfixToPostFixConvertor convertor = new InfixToPostFixConvertor();
		convertor.perform("3 * Non-Existant-Operation 45");
	}
